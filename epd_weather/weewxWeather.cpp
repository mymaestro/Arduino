...
String AirliftweewxWeather::buildUrlCurrent(String appId, String location) {
...
String AirliftweewxWeather::buildUrlForecast(String appId, String location) {
  String units = OWM_METRIC ? "metric" : "imperial";
  return "http://api.openweathermap.org/data/2.5/forecast?q=" + location + "&cnt=6&appid=" + appId + "&units=" + units + "&lang=" + String(OWM_LANGUAGE);
}

String AirliftweewxWeather::getMeteoconIcon(String icon) {
  // clear sky
  // 01d
  if (icon == "01d") 	{
    return "B";
...
bool AirliftweewxWeather::updateCurrent(weewxWeatherCurrentData &data, String json)
{
  ...
  if(code != 200)
  {
    Serial->println(String("weewxWeather error: ") + (const char *)doc["message"]);
    setError(String("weewxWeather error: ") + (const char *)doc["message"]);
    return false;
  }
  ...
  data.timezone = (uint8_t) doc["timezone"];
  ...
  data.temperature = (float) doc["main"]["temp"];
  ...
  Serial->print("Epoch: ");
  Serial->println((unsigned long) data.observationTime);
  Serial->print("Time zone: ");
  Serial->println((uint8_t)data.timezone);
  return true;
}

bool AirliftweewxWeather::updateForecast(weewxWeatherForecastData &data, String json, int day)
{
  ...
  data.temperature = (float) doc["list"][day]["main"]["temp"];
